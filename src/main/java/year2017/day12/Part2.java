...
public class Part2 {
	...
	public static void main(String[] args) {
		try (Stream<String> lines = new BufferedReader(new InputStreamReader(Part2.class.getResourceAsStream("input")))
				.lines()) {
			System.out.println(numberOfGroups(lines));
		}
	}

	static int numberOfGroups(Stream<String> lines) {
		...
		MutableIntSet ids = IntHashSet.newSet(programs.keySet());
		int result = 0;

		while (!ids.isEmpty()) {
			boolean[] added = { false };
			MutableIntSet zeroGroup = new IntHashSet();
			zeroGroup.addAll(programs.get(ids.intIterator().next()).connections);
			do {
				added[0] = false;
